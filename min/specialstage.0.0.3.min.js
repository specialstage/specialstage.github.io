...
80;var E=128;var H=m;var Q=128;var K=80;var da=128;var Y=m+60;var Z=128;var aa=40;var N=!1,ba=[],U=[];this.connect=function(){f.imageSmoothingEnabled=!1;q.width=w.width;q.height=w.height;J.drawImage(w,0,0,q.width,q.height,0,0,q.width,q.height)};this.resize=function(t,x,y){var B=document.querySelector("meta[name=viewport]"),I=1/window.devicePixelRatio*Math.round(window.devicePixelRatio);B.setAttribute("content","width="+Math.floor(window.innerWidth*I)+", initial-scale="+I+",maximum-scale=4.0, user-scalable=0");
a.scale=y;this.width=t;this.height=x;this.renderer.width=t;this.renderer.height=x;this.renderer.style.zoom=y;f.imageSmoothingEnabled=!1;this.redraw();b=128;m=180;n=128;k=80;E=128;H=m;Q=128;K=80;da=128;Y=m+60;Z=128;aa=40;this.automate(t,x)};this.update=function(t){for(var x in t)for(var y in U)if(t[x].x>U[y].x*a.scale&&t[x].y>U[y].y*a.scale&&t[x].x<U[y].w*a.scale&&t[x].y<U[y].h*a.scale&&1==t[x].start){U[y].funct();t[x].start=!1;break}};this.print=function(t,x,y,B){B=void 0===B?1:B;for(var I=0,V=a.compile(x,
a.width,a.col),F=a.compile(y,a.height,a.row);I<t.length;)a.encode(t.charAt(I),V+I*a.col*B,F,B),I++;N||ba.push(function(){a.print(t,x,y,B)})};this.text=function(t,x,y,B,I){B=void 0===B?1:B;for(I=0;I<t.length;)a.encode(t.charAt(I),x+I*a.col*B,y,B),I++};this.button=function(t,x,y,B,I,V,F,D){F=void 0===F?!0:F;D=void 0===D?1:D;var M=a.compile(x,a.width,a.col),S=a.compile(y,a.height,a.row),ca=a.compile(B,a.width,a.col),P=a.compile(I,a.height,a.row);F&&(f.drawImage(q,0,0,1,1,M,S+P/2,ca+1,1),f.drawImage(q,
0,0,1,1,M,S-P/2,ca+1,1),f.drawImage(q,0,0,1,1,M,S-P/2,1,P),f.drawImage(q,0,0,1,1,M+ca-1,S-P/2,1,P));for(var ha=0;ha<t.length;)a.encode(t.charAt(ha),M+ha*a.col+ca/2-Math.round(t.length*a.col)/2+2,S-a.row/2,D),ha++;U.push({x:M,y:S-P/2,w:M+ca,h:S+P/2,funct:function(){V()}});N||ba.push(function(){a.button(t,x,y,B,I,V,F,D)})};this.line=function(t,x,y,B){a.sprite(0,0,1,1,t,x,y,B)};this.sprite=function(t,x,y,B,I,V,F,D){var M=a.compile(I,a.width,a.col),S=a.compile(V,a.height,a.row),ca=a.compile(F,a.width,
a.col),P=a.compile(D,a.height,a.row);f.drawImage(q,t,x,y,B,M,S,ca,P);N||ba.push(function(){a.sprite(t,x,y,B,I,V,F,D)})};this.backdrop=function(t,x,y,B,I){t=void 0===t?"#10101099":t;!1===t?(a.clear(),a.renderer.style.background="transparent"):(a.clear(),a.renderer.style.background=t)};this.getDrawList=function(){return ba};this.redraw=function(){N=!0;a.clear();for(var t=0;t<ba.length;t++)ba[t]();t=document.body.getElementsByTagName("input");for(var x=0;x<t.length;x++)a.text(t[x].value,t[x].style.top,
t[x].style.left);N=!1};this.automate=function(t,x){var y=Math.floor(t/3)-2,B=Math.floor(x/4);n>y&&(da=Z=E=Q=b=n=y);m>B&&(H=m=B+40,Y=m+60)};this.pad=function(t,x,y,B,I,V,F){var D=0;for(F();D<control.touches.length;)control.touches[D].x>x*a.scale&&control.touches[D].y>y*a.scale&&control.touches[D].x<(x+B)*a.scale&&control.touches[D].y<(y+I)*a.scale&&V(),D++;HITBOX&&(f.drawImage(q,0,0,1,1,x+1,y,B-1,1),f.drawImage(q,0,0,1,1,x+1,y+I,B-1,1),f.drawImage(q,0,0,1,1,x+1,y,1,I),f.drawImage(q,0,0,1,1,x+B-1,y,
1,I));a.encode(t,x+B/2-a.col/2,y+I/2-a.row/2)};this.dpad=function(){f.drawImage(q,88,88,40,40,a.width-E+Q/2-20,a.height-H+K/2-20,40,40);f.drawImage(q,88,70,40,17,a.width-da+Q/2-20,a.height-Y+aa-8,40,17);a.pad("<",b-n-1,a.height-m,n,k,function(){control.LEFT=!0},function(){control.LEFT=!1});a.pad(">",b+1,a.height-m,n,k,function(){control.RIGHT=!0},function(){control.RIGHT=!1});a.pad("^",a.width-E,a.height-H,Q,K,function(){control.UP=!0},function(){control.UP=!1});a.pad("b",a.width-da,a.height-Y+aa/
2,Z,aa,function(){control.DOWN=!0},function(){control.DOWN=!1})};this.clear=function(){if(!N){for(var t=document.body.getElementsByTagName("input"),x=0;x<t.length;x++)document.body.removeChild(t[x]);ba=[]}U=[];f.clearRect(0,0,a.width,a.height)};this["delete"]=function(t,x,y,B){x=void 0===x?0:x;y=void 0===y?renderer.width:y;B=void 0===B?renderer.height:B;t=a.compile(void 0===t?0:t,a.width,a.col);x=a.compile(x,a.height,a.row);y=a.compile(y,a.width,a.col);B=a.compile(B,a.height,a.row);f.clearRect(t,
...
t();break;case "string":for(var I=!1,V=1,F=0,D="";F<t.length;){var M=t.charAt(F);"."===M?(D+=M,I=!0):"+"===M?(D=I?Number(D)*x:Number(D)*y,D*=V,V=1,B+=D,I=!1,D=""):"-"===M?(D=I?Number(D)*x:Number(D)*y,D*=V,V=-1,B+=D,I=!1,D=""):"p"===M?(D=Number(D),D*=V,B+=D,D=""):F===t.length-1?(D+=M,D=I?Number(D)*x:Number(D)*y,D*=V,B+=D,D=""):D+=M;F++}}return B};this.textinput=function(t,x,y,B,I,V,F){var D=a.compile(x,a.width,a.col),M=a.compile(y,a.height,a.row),S=a.compile(B,a.width,a.col),ca=a.compile(I,a.height,
a.row);if(N)console.log(F.value),a.text("> "+F.value,D,M+ca/2);else{F=document.createElement("input");F.type="text";F.id=V;F.name=V;F.value=t;F.style.top=M*a.scale+"px";F.style.left=D*a.scale+"px";F.style.width=S*a.scale+"px";F.style.height=ca*a.scale+"px";F.style.paddingLeft=12*a.scale+"px";F.style.fontSize=14*a.scale+"px";F.maxlength=10;F.autofocus=!0;a.text("> "+F.value,D,M+ca/2);document.body.appendChild(F);F.focus();F.oninput=function(){P()};F.onfocusout=function(){P()};F.onfocusin=function(){P()};
F.onclick=function(){P()};var P=function(ha){a["delete"](D,M,S,ca);a.text("> "+F.value,D,M+ca/2);a.text("_",F.selectionStart*a.col+2*a.col+D,M+ca/2)};ba.push(function(){a.textinput(F.value,x,y,B,I,V,F)})}}};function Control(){var a=this;this.ENTER=this.RIGHT=this.LEFT=this.DOWN=this.UP=this.MOBILE=!1;this.touches=[];this.connect=function(){this.MOBILE?(window.addEventListener("touchstart",a.touch,!1),window.addEventListener("touchmove",a.touch,!1),window.addEventListener("touchend",a.touch,!1)):(window.addEventListener("keydown",a.key,!1),window.addEventListener("keyup",a.key,!1),window.addEventListener("mousedown",a.mousedown),window.addEventListener("mouseup",a.mouseup,!1),window.addEventListener("touchstart",
...
this.DOWN=this.UP=!1};this.touchstart=function(f){f=f.touches;a.touches=[];for(var w=0;w<f.length;w++)a.touches[w]={x:f[w].clientX,y:f[w].clientY,start:!0}};this.touchmove=function(f){a.touches=[];f=f.touches;for(var w=0;w<f.length;w++)a.touches[w]={x:f[w].clientX,y:f[w].clientY,start:!1}};this.touchend=function(f){a.touches=[];f=f.touches;for(var w=0;w<f.length;w++)a.touches[w]={x:f[w].clientX,y:f[w].clientY,start:!1}}};function Vehicle(){function a(){if(w)for(var z=0;1>z;){emitter.geometry.vertices[ea].copy(S);emitter.geometry.vertices[ea].multiplyScalar(-1-Math.random()*V);emitter.geometry.vertices[ea].add(l);emitter.geometry.vertices[ea].life=emitter.geometry.vertices[ea].maxLife;var W=.1*-Math.random();emitter.geometry.vertices[ea].acc.copy(S);emitter.geometry.vertices[ea].acc.multiplyScalar(W);ea++;ea%=emitter.geometry.vertices.length;z++}for(var fa in emitter.geometry.vertices)0<emitter.geometry.vertices[fa].life&&
...
var f=this;this.TEXTTIME=this.SPEED="";this.END=this.START=!1;this.OBJECTIVE=0;this.POSITION;this.MODEL=1;this.ANALOG=!1;this.PENALTY=function(){return E};this.CAMERA=1;var w=!1,q=!1,J=!0,b=!1,m=1,n=0,k=[],E=0,H=0,Q=0,K=0,da=!1,Y=0,Z=0,aa=0,N={power:2,drag:2,mass:1,brake:3,sensitivity:2,ease:0},ba=0,U=0,t=0;ba=.02-.015;U=.985-.98;t=.075-.055;var x,y,B,I=6,V,F=0,D=0,M=0,S=new THREE.Vector3(0,1,0),ca=new THREE.Vector3(0,0,0),P=new THREE.Vector3,ha=new THREE.Vector3(0,0,1),ja=new THREE.Vector3(0,0,1),
...
x:-40,y:40,z:30,lerp:.075,lookAt:new THREE.Vector3};this.cameras[4]={distance:0,x:0,y:-20,z:60,lerp:.0075,lookAt:new THREE.Vector3};var A=[];this.models=function(){return A};A[0]=(new THREE.Geometry).fromBufferGeometry(new THREE.EdgesGeometry(new THREE.BoxGeometry(1,2,1)));A[0].name="logistics crate";var T=new THREE.MeshBasicMaterial;A[1]=new THREE.Geometry;A[1].vertices.push(new THREE.Vector3(.45,-.75,1));A[1].vertices.push(new THREE.Vector3(.3,1.2,1));A[1].vertices.push(new THREE.Vector3(-.45,-.75,
1));A[1].vertices.push(new THREE.Vector3(-.3,1.2,1));A[1].vertices.push(new THREE.Vector3(.3,1.2,1));A[1].vertices.push(new THREE.Vector3(-.3,1.2,1));A[1].vertices.push(new THREE.Vector3(.45,-.75,1));A[1].vertices.push(new THREE.Vector3(-.45,-.75,1));A[1].vertices.push(new THREE.Vector3(-.3,1.2,1));A[1].vertices.push(new THREE.Vector3(-1,-.75,-.1));A[1].vertices.push(new THREE.Vector3(-.45,-.75,1));A[1].vertices.push(new THREE.Vector3(-1,-.75,-.1));A[1].vertices.push(new THREE.Vector3(.3,1.2,1));
A[1].vertices.push(new THREE.Vector3(1,-.75,-.1));A[1].vertices.push(new THREE.Vector3(.45,-.75,1));A[1].vertices.push(new THREE.Vector3(1,-.75,-.1));A[1].vertices.push(new THREE.Vector3(.45,-.75,1));A[1].vertices.push(new THREE.Vector3(.45,-1.4,.4));A[1].vertices.push(new THREE.Vector3(-.45,-.75,1));A[1].vertices.push(new THREE.Vector3(-.45,-1.4,.4));A[1].vertices.push(new THREE.Vector3(.45,-1.4,.4));A[1].vertices.push(new THREE.Vector3(-.45,-1.4,.4));A[1].vertices.push(new THREE.Vector3(.45,-1.4,
.4));A[1].vertices.push(new THREE.Vector3(1,-.75,-.1));A[1].vertices.push(new THREE.Vector3(-.45,-1.4,.4));A[1].vertices.push(new THREE.Vector3(-1,-.75,-.1));A[1].center();A[1].name="ag-86";for(var O=(new THREE.LineSegments(A[f.MODEL].clone(),T)).clone(),ea=0,ia=new THREE.Geometry;100>ia.vertices.length;){var X=new THREE.Vector3(0,0,0);X.life=0;X.maxLife=100;X.acc=new THREE.Vector3(0,0,0);ia.vertices.push(X);X=new THREE.Color(4294967040);ia.colors.push(X)}ia.verticesNeedUpdate=!0;emitter=new THREE.Points(ia,
new THREE.PointsMaterial({size:.1,vertexColors:THREE.VertexColors,blending:THREE.AdditiveBlending,transparent:!0}));emitter.name="Emitter";this.init=function(){var z=state.cLoad("YEN");console.log(z);Z=""===z?0:parseInt(z);console.log(Z)};this.connect=function(z,W){O.copy(new THREE.LineSegments(A[f.MODEL].clone(),T));h.copy(z);l.copy(h);p.copy(l);scene.add(O);scene.add(emitter);f.tune.power();f.tune.drag();f.tune.sensitivity();f.tune.ease();scene.fog=new THREE.FogExp2(palette[0],.01);camera.fov=90;
camera.updateProjectionMatrix();scene.add(stage.generate.stars);b=!1;window.setTimeout(function(){b=!0},1E3)};this.disconnect=function(){f.reset();scene.remove(O);scene.remove(emitter);scene.fog=new THREE.FogExp2(palette[0],0);scene.remove(stage.generate.stars)};this.ready=function(){w&&(K=performance.now(),n=0,f.START=!0)};this.reset=function(){if(f.END&&k[3]+E<stage.best[3])for(var z in stage.best)stage.best[z]=3===z?n+E:k[z];b=DNF=this.END=this.START=!1;control.UP=!1;control.DOWN=!1;control.LEFT=
!1;R=L=q=w=control.RIGHT=!1;n=0;k=[];aa=H=E=Y=0;l.copy(h);p.copy(l);S.set(0,1,0);P.set(0,0,0);d.set(0,0,1);O.copy(new THREE.LineSegments(A[f.MODEL].clone(),T));O.lookAt(d);F=D=0;camera.position.copy(l);camera.position.add(S.clone().multiplyScalar(-60));camera.position.z+=40;camera.lookAt(O.position);camera.updateProjectionMatrix();renderer.render(scene,camera);vhs.reset();window.setTimeout(function(){b=!0},1E3)};this.update=function(){this.START?control.RESET&&this.reset():this.ready();DNF||f.END||
!b?R=L=q=w=!1:(w=control.UP,q=control.DOWN,L=control.LEFT,R=control.RIGHT);a();var z=L&&R?0:L?1:R?-1:0;0!=z&&Math.abs(M)<I?M+=z:0<Math.abs(M)&&(M-=M/Math.abs(M));F=M/I;F*=B;D+=F;D%=2*Math.PI;S.copy(O.up);S.applyMatrix4(O.matrixWorld);S.sub(l);S.normalize();ca.copy(S);V=P.length();w&&J&&(ca.copy(S).multiplyScalar(x),P.add(ca));q&&J&&P.multiplyScalar(.95);J?P.multiplyScalar(y):P.multiplyScalar(.99);l.add(P);O.position.copy(l);C.copy(l);C.z-=50;r=[];u.set(0,0,1);G.far=100;G.set(C,u);stage.surface.raycast(G,
r);0<r.length?(C.copy(r[0].point),l.z<C.z&&5>Math.abs(l.z-C.z)?(Q=0,J=!0,ja.copy(r[0].face.normal),l.copy(C)):J?(J=!1,P.sub(g)):P.add(g)):0===r.length&&(J=!1,Q++,P.add(g),60<Q&&!f.END&&!DNF&&(DNF=!0,state.dnf()));e.lerp(ja,.2);d.copy(e);d.add(l);O.lookAt(d);O.rotateOnAxis(ha,D);r=[];u.copy(l);u.sub(p);G.far=u.length();u.normalize();G.set(l,u);z=0;for(var W=!1,fa=aa;5>fa;fa++)if(stage.generate.checkpoints[fa].collision.raycast(G,r),0<r.length){z=fa;W=!0;break}da=!1;if(W)if(z===aa)stage.generate.checkpoints[aa].display.material.color=
palette[4],0<aa&&(k[aa-1]=n,ui["delete"]("2","4","30","1"),W=Number.isNaN(stage.best[aa-1])?"+1 cp":n-stage.best[aa-1],W=0<W?"(-"+ui["float"](W)+")":"+1 cp"===W?"+1 cp":"(+"+ui["float"](Math.abs(W))+")",ui.text(W,16,32)),4===aa&&(da=!0,k[z-1]=n,k[z]=n+E,Y=n+E<stage.best[3]?100-E+Math.floor(10*(stage.best[3]-(n+E))):100-E,Z+=Y,f.END=!0,state.complete()),da=!0,aa++;else{da=!0;for(W=aa-1;W<z-1;W++)0<=W&&(k[W]="-",H++);E=25*H;4===z?(k[z-1]=n,k[z]=n+E,Y=n+E<stage.best[3]?100-E+Math.floor(10*(stage.best[3]-
(n+E))):100-E,Z+=Y,aa=0,f.END=!0,state.complete()):(k[z-1]=n,aa=z+1,ui["delete"]("2","4","30","1"),ui.text("missed checkpoint x "+H.toString()+" ( -"+E.toString()+".00 )",16,32));da=!0;stage.generate.checkpoints[z].display.material.color=palette[4]}f.SPEED=Math.round(60*V/1E3*3600);f.START&&!f.END&&(n=Math.floor((window.performance.now()-K)/10)/100);f.OBJECTIVE=aa;p.copy(l);z=DNF||f.END?1E-5:.1;W=S.clone();W.multiplyScalar(-f.cameras[0].distance);W.add(O.position);W.z+=f.cameras[0].z;camera.position.lerp(W,
z);camera.lookAt(O.position)};this.results=function(){return{reward:Y,yen:Z}};this.AT=function(){return n};this.CP=function(){return k.slice()};this.replay=function(z){l.copy(z.position);O.position.copy(l);S.copy(z.direction);ja.copy(z.normal);d.copy(z.lookAt);w=z.UP;da=z.CHECK;D=z.rotation;aa=z.objective;O.lookAt(d);O.rotateOnAxis(ha,D);z=l.clone();var W=1,fa=1;f.CAMERA===m&&(W=f.cameras[1].lerp,fa=1);m=f.CAMERA;z.x+=f.cameras[f.CAMERA].x;z.y+=f.cameras[f.CAMERA].y;z.z+=f.cameras[f.CAMERA].z;camera.position.lerp(z,
W);f.cameras[m].lookAt.lerp(l,fa);camera.lookAt(f.cameras[m].lookAt);da&&(stage.generate.checkpoints[aa].display.material.color=palette[4]);a()};this.record=function(){return{position:l.clone(),direction:S.clone(),normal:ja.clone(),lookAt:d.clone(),UP:w,rotation:D,objective:aa,check:da}};this.tune={power:function(z){z=void 0===z?N.power:z;N.power=0>z?0:5<z?5:z;x=.015+N.power/5*ba},drag:function(z){z=void 0===z?N.drag:z;N.drag=0>z?0:5<z?5:z;y=.985-N.drag/5*U},sensitivity:function(z){z=void 0===z?N.sensitivity:
z;N.sensitivity=0>z?0:5<z?5:z;B=.055+N.sensitivity/5*t},ease:function(z){z=void 0===z?N.ease:z;N.ease=0>z?0:5<z?5:z;I=6+N.ease}};this.param=function(){array=Object.assign(N,[]);array.model=A[f.MODEL].name;return array};this.yen=function(){state.cSave("YEN",Z);return Z};this.debug=function(){console.log("ROTATION:",D)}};function Generate(){scope=this;var a=this.SEED=0;this.length=480;var f=[],w=0;f[0]={name:"forest",palette:palette[9],tree:function(e,d){return scope.pine()},limit:Math.floor(scope.length/2),range:5,map:[],flags:[]};f[0].map[0]=function(e,d){var g=palette[10],l=!1;.92<Math.abs(e)&&(g=.92<Math.abs(e)?palette[9]:palette[2],l=!0);f[0].flags[d]=l;return g};f[1]={name:"sakura",palette:palette[12],tree:function(){return scope.sakura()},limit:Math.floor(1/6*scope.length),range:10,map:[],flags:[]};f[1].map[0]=
...
"Collision Material";K.material.side=THREE.BackSide;this.checkpoints=[];var da=new THREE.Vector3(0,0,1),Y=new THREE.Vector3(1,0,0);new THREE.Vector3(0,1,0);new THREE.Vector3(0,0,1);var Z=new THREE.Vector3(0,3,0),aa=new THREE.Vector3(0,0,1),N=J.geometry.clone(),ba=m.geometry.clone(),U=b.geometry.clone(),t=n.geometry.clone(),x=[],y=[],B=new THREE.Vector3;x[0]=new THREE.Vector3(4,3,0);x[1]=new THREE.Vector3(-4,3,0);y[0]=x[0].clone();y.z+=1;y[1]=x[1].clone();y.z+=1;var I=Math.PI/64,V=Math.PI/32,F=[],
D=0,M=new THREE.Vector3;this.start=new THREE.Vector3;var S=!1,ca=!1,P=!1;this.IMPORT=this.END=!1;this.DISTANCE=0;var ha=18,ja=new THREE.Raycaster;ja.far=800;q.geometry.vertices[0]=Z.clone();q.geometry.vertices[1]=da.clone().add(Z);q.geometry.vertices[2]=x[0].clone();q.geometry.vertices[3]=x[1].clone();q.geometry.vertices[4]=y[0].clone();q.geometry.vertices[5]=y[1].clone();this.connect=function(){scope.SEED=SEED;w=scope.randomInt(0,2);scene.add(q);scene.add(J);scene.add(b);scene.add(n);scene.add(m);
...
Q.geometry.dispose();U.dispose();t.dispose();N.dispose();ba.dispose();U.copy(new THREE.Geometry);t.copy(new THREE.Geometry);N.copy(new THREE.Geometry);ba.copy(new THREE.Geometry);J.geometry.copy(new THREE.Geometry);q.geometry.copy(new THREE.Geometry);b.geometry.copy(new THREE.Geometry);n.geometry.copy(new THREE.Geometry);m.geometry.copy(new THREE.Geometry);k.geometry.copy(new THREE.Geometry);K.geometry.copy(new THREE.Geometry);Z.set(0,3,0);aa.set(0,0,1);x[0]=new THREE.Vector3(4,3,0);x[1]=new THREE.Vector3(-4,
3,0);S&&(P=ca=S=!1);this.END=!1;q.position.set(0,0,0);q.geometry.vertices[0]=Z.clone();q.geometry.vertices[1]=da.clone().add(Z);q.geometry.vertices[2]=x[0].clone();q.geometry.vertices[3]=x[1].clone();q.geometry.vertices[4]=y[0].clone();q.geometry.vertices[5]=y[1].clone();q.geometry.verticesNeedUpdate=!0};this.generate=function(){scope.path()};this.path=function(e,d,g,l,p,h,C,r,u,G){e=void 0===e?0:e;d=void 0===d?0:d;g=void 0===g?0:g;l=void 0===l?0:l;p=void 0===p?0:p;h=void 0===h?{start:0,end:30,angle:0,
slope:0}:h;C=void 0===C?[]:C;r=void 0===r?[]:r;u=void 0===u?!1:u;G=void 0===G?999:G;var A=window.performance.now(),T=0;for(0===l&&(l=.5<scope.random()?1:-1);32>T&&!ca;){if(e===h.end+1){r=[];for(g=0;1>g;g++)if(e>=scope.length)r.push({angle:0,slope:0,end:30}),G=e+30;else if(0===d){T=scope.randomInt(2,4)*l;var O=Math.floor(Math.abs(36/T));l=.01>scope.random()?1*l:-1*l;O=scope.randomInt(1,O);r.push({angle:T,slope:0,end:O})}else T=scope.randomInt(-5,-1),T=.2>scope.random()?T*=-1:T,O=scope.randomInt(2,
20),r.push({angle:0,slope:T,end:O});T=scope.randomInt(0,r.length);d=r[T].angle;g=r[T].slope;h.start=e;h.end=e+r[T].end;r=r.slice(T,1)}u||(scope.extrude(d,g),F.push({angle:d,slope:g}),N.verticesNeedUpdate=!0,e++);if(10<=e&&!u){T=new THREE.Vector2(N.vertices[e-1].x,N.vertices[e-1].y);for(O=0;O<N.vertices.length-1-10;O++)if(18>(new THREE.Vector2(N.vertices[O].x,N.vertices[O].y)).distanceTo(T)){u=!0;break}u&&window.setTimeout(stage.reset(!0),0)}e===G&&(ca=!0,J.geometry.dispose(),J.geometry=N.clone(),
J.geometry.computeBoundingSphere(),J.geometry.center(),J.updateMatrixWorld(),0<J.geometry.vertices.length&&(M.copy(J.geometry.vertices[0]),M.sub(N.vertices[0])),window.setTimeout(function(){scope["import"]()},0));T=window.performance.now()-A}ca||u||window.setTimeout(scope.path,0,e,d,g,l,p,h,C,r,u,G)};this["import"]=function(e){e=void 0===e?0:e;for(var d=window.performance.now(),g=0;!P&&32>g;){0===e&&(q.position.set(0,0,0),q.rotation.z=0,Z.set(0,3,0),q.geometry.vertices[0]=Z.clone(),q.geometry.vertices[1]=
da.clone().add(Z),q.geometry.vertices[2]=x[0].clone(),q.geometry.vertices[3]=x[1].clone(),q.geometry.vertices[4]=y[0].clone(),q.geometry.vertices[5]=y[1].clone(),q.geometry.verticesNeedUpdate=!0,q.updateMatrixWorld());if(e<F.length)angle=F[e].angle,slope=F[e].slope,scope.extrude(angle,slope),b.geometry.dispose(),b.geometry=U.clone(),b.geometry.computeBoundingSphere(),n.geometry.dispose(),n.geometry=t.clone(),n.geometry.computeBoundingSphere(),m.geometry.dispose(),m.geometry=ba.clone(),m.geometry.computeBoundingSphere(),
...
var l=q.geometry.vertices[2].clone(),p=q.geometry.vertices[3].clone(),h=q.geometry.vertices[1].clone();g.applyMatrix4(q.matrix);l.applyMatrix4(q.matrix);p.applyMatrix4(q.matrix);h.applyMatrix4(q.matrix);h.sub(g);if(ca){l.add(M);p.add(M);if(0<U.vertices.length){var C=U.vertices[U.vertices.length-1].clone().sub(B),r=U.vertices[U.vertices.length-2].clone().sub(B);ba.vertices.push(l.clone().sub(h));ba.vertices.push(p.clone().sub(h));ba.vertices.push(r);ba.vertices.push(p.clone().sub(h));ba.vertices.push(C);
ba.vertices.push(r);C=l.clone();r=p.clone();var u=U.vertices[U.vertices.length-1].clone(),G=U.vertices[U.vertices.length-2].clone();K.geometry.vertices.push(C);K.geometry.vertices.push(r);K.geometry.vertices.push(G);K.geometry.vertices.push(r);K.geometry.vertices.push(u);K.geometry.vertices.push(G);C=ba.vertices.length;ba.faces.push(new THREE.Face3(C-3,C-2,C-1));ba.faces.push(new THREE.Face3(C-5,C-4,C-6));K.geometry.faces.push(new THREE.Face3(C-3,C-2,C-1));K.geometry.faces.push(new THREE.Face3(C-
...
U.vertices.push(p.clone());B.copy(h)}else N.vertices.push(g.clone());Z.copy(g).sub(q.position);q.position.add(Z);for(var A in q.geometry.vertices)q.geometry.vertices[A].applyAxisAngle(Y,-1*d*I)};this.terrain=function(e,d,g,l,p,h,C){d=void 0===d?[]:d;g=void 0===g?[]:g;l=void 0===l?[]:l;p=void 0===p?0:p;h=void 0===h?0:h;C=void 0===C?0:C;if(void 0===e||e){b.updateMatrixWorld();k.geometry.dispose();k.geometry.copy(new THREE.Geometry);d=[];g=[];l=[];for(e=0;2>e;e++)l[e]=[],d[e]=[],g[e]=[];for(e=0;e<b.geometry.vertices.length;e++)g[e%
2].push(b.geometry.vertices[e]);for(e=0;2>e;e++)for(var r=0;r<g[e].length;r++){var u=0===e?1:-1,G=g[e][r].clone();G.sub(g[e+u][r]);G.normalize().multiplyScalar(3);l[e][r]=G.clone()}}r=0;e=window.performance.now();if(4>p){k.geometry.verticesNeedUpdate=!0;k.geometry.elementsNeedUpdate=!0;k.geometry.computeBoundingSphere();k.geometry.computeFaceNormals();k.geometry.computeVertexNormals();k.updateMatrixWorld();if(0<p&&0===C)for(g[h]=[],u=0;u<d[h].length;u++)g[h][u]=k.geometry.vertices[d[h][u]];0===C&&
(d[h]=[]);for(v=C;v<g[h].length&&32>r;){r=!1;if(void 0!==g[h][v]&&void 0!==g[h][v-1]){if(void 0===l[h][v])l[h][v]=l[s][v-1].clone();else if(k.geometry.computeBoundingSphere(),u=g[h][v].clone().add(l[h][v]),u.z-=100,ja.set(u,da),u.z+=100,G=ja.intersectObjects([k],!0),0<G.length)for(var A in G).01<G[A].point.distanceTo(u)&&(d[h][v]=void 0,r=!0);r||(k.geometry.vertices.push(g[h][v].clone()),k.geometry.vertices.push(g[h][v].clone().add(l[h][v])),d[h][v]=k.geometry.vertices.length-1,0<v&&(k.geometry.vertices.push(g[h][v-
1].clone()),r=k.geometry.vertices.length,0===h?k.geometry.faces.push(new THREE.Face3(r-2,r-1,r-3)):1===h&&k.geometry.faces.push(new THREE.Face3(r-1,r-2,r-3))))}r=window.performance.now()-e;v++;C=v}if(C===g[h].length){C=0;for(A=1;A<d[h].length;A++)void 0!==d[h][A]&&void 0!==d[h][A-1]&&(e=k.geometry.vertices[d[h][A]],r=k.geometry.vertices[d[h][A-1]],2.8>e.distanceTo(r)?(e.copy(r),d[h][A]=d[h][A-1],l[h][A].copy(l[h][A-1])):1<A&&(k.geometry.vertices.push(g[h][A-1]),k.geometry.faces.push(new THREE.Face3(0===
h?d[h][A]:d[h][A-1],0===h?d[h][A-1]:d[h][A],k.geometry.vertices.length-1))));h++}1<h&&(h=0,p++)}if(4===p){k.geometry.verticesNeedUpdate=!0;k.geometry.elementsNeedUpdate=!0;k.geometry.colorsNeedUpdate=!0;k.geometry.computeFaceNormals();k.geometry.computeVertexNormals();k.geometry.mergeVertices();k.geometry.computeBoundingSphere();scope.noise(k.geometry);for(A=0;A<k.geometry.faces.length;A++)e=k.geometry.vertices[k.geometry.faces[A].a].clone(),r=k.geometry.vertices[k.geometry.faces[A].b].clone(),u=
k.geometry.vertices[k.geometry.faces[A].c].clone(),K.geometry.vertices.push(e),K.geometry.vertices.push(r),K.geometry.vertices.push(u),wireframe.geometry.vertices.push(e),wireframe.geometry.vertices.push(r),wireframe.geometry.vertices.push(u),e=K.geometry.vertices.length,K.geometry.faces.push(new THREE.Face3(e-1,e-2,e-3)),wireframe.geometry.faces.push(new THREE.Face3(e-1,e-2,e-3));wireframe.geometry.copy(k.geometry);wireframe.geometry.computeVertexNormals();scope.biome();window.setTimeout(scope.complete,
...
[];for(u=0;u<g.length;u++)for(var A=G=0;3>A;A++){var T=0===r&&2===A&&1>G?1:scope.random();if(.2<T){T=scope.randomInt(-2,2);var O=g[u].n.clone(),ea=palette[2];O.applyAxisAngle(g[u].pitch,2*Math.PI/(C+T));O.applyAxisAngle(g[u].roll,2*Math.PI/3*A);O.multiplyScalar(l);T=O.clone().add(g[u].v);e.vertices.push(g[u].v.clone());e.vertices.push(T);e.colors.push(ea);e.colors.push(ea);ea=g[u].pitch.clone();ea.applyAxisAngle(g[u].roll,2*Math.PI/3*A);ea.applyAxisAngle(g[u].pitch,2*Math.PI/C);var ia=g[u].roll.clone();
ia.applyAxisAngle(g[u].pitch,2*Math.PI/C);ia.applyAxisAngle(g[u].roll,2*Math.PI/3*A);0<A&&d.push({a:T.clone(),b:g[u].v.clone(),pitch:ea,roll:ia});h.push({v:T.clone(),n:O.clone(),pitch:ea,roll:ia});G++}else 0===G&&2===A&&p.push({v:g[u].v.clone(),n:g[u].n.clone(),pitch:g[u].pitch.clone(),roll:g[u].roll.clone()})}g=h.slice();l*=.9;r++}C=6;for(var X in d)l=d[X].a.clone(),r=d[X].b.clone(),l=l.clone().sub(r),h=l.clone().multiplyScalar(.5).add(r),r=scope.randomInt(0,3)-1,l.applyAxisAngle(d[X].pitch,2*Math.PI/
...
vehicle.tune.drag(n);vehicle.tune.sensitivity(k);vehicle.tune.ease(E)}};var BUILD="0.0.2_1 _ DEV 1";window.onload=init;window.onresize=resize;window.fullscreenchange=resize;window.focus=resize;var palette=[1052688,16777215,12698049,2894892,5308334,16740978,2393855,16722266,3424820,2201152,7168071,10066329,16756970],i;for(i in palette){var c=new THREE.Color(palette[i]);palette[i]=c}
