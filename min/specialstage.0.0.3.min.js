...
a.encodeURL();window.setTimeout(a.results,2E3)})};this.inputName=function(b){ui.clear();a.nav(function(){b()},a.menu);ui.print("edit operator name:","2","2");ui.textinput(OPERATOR_ID,"2","4","1.0-2","6","operator_id");var m=document.getElementById("operator_id");ui.button("accept","2","14","1.0-4","6",function(){OPERATOR_ID=m.value.toLowerCase();a.cSave("OPERATOR_ID",OPERATOR_ID);b()},!0)};this.encodeURL=function(){data=[];data[0]={operator_id:OPERATOR_ID,seed:stage.name,cp:[]};for(var b=vehicle.CP();5>
data[0].cp.length;)data[0].cp.push(ui["float"](b[data[0].cp.length]));if(CHALLENGE)for(data[1]={operator_id:a.CHALLENGES[0].operator_id,seed:stage.name,cp:[]};5>data[1].cp.length;)data[1].cp.push(ui["float"](a.CHALLENGES[0].cp[data[1].cp.length]));b="";for(var m=0;m<data.length;m++)0<m&&(b+="\n"),b+=data[m].operator_id,b+="\t",b+=data[m].seed,b+="\t",b+=data[m].cp[0],b+="\t",b+=data[m].cp[1],b+="\t",b+=data[m].cp[2],b+="\t",b+=data[m].cp[3],b+="\t",b+=data[m].cp[4];b=window.btoa(b);window.history.pushState("",
"","/");m=window.location.href;var n=window.location.pathname+"#"+b;navigator.clipboard.writeText(m+"#"+b).then(function(){ui.print("url copied to clipboard","2","2");window.history.pushState("","",n)},function(){window.history.pushState("","",n);ui.print("url copied to address bar","2","2")});FIREBASE&&firebase.analytics().logEvent("challenge_issued",{url:m+"#"+b})};this.decodeURL=function(){for(var b=window.location.href,m="",n=!1,k=0;k<b.length;k++)n&&(m+=b.charAt(k)),"#"===b.charAt(k)&&(n=!0);
if(n){var E=k=0;n=[[],[]];n[E][k]="";try{var H=window.atob(m);for(m=0;m<H.length;m++){var Q=H.charAt(m);"\t"===Q?(k++,n[E][k]=""):"\n"===Q?(k=0,E++,n[E][k]=""):n[E][k]+=Q}for(H=0;H<n.length;H++)7===n[H].length&&(a.CHALLENGES[H]={id:"",seed:"",cp:[]},a.CHALLENGES[H].operator_id=n[H][0],a.CHALLENGES[H].seed=n[H][1],a.CHALLENGES[H].cp[0]=parseFloat(n[H][2]),a.CHALLENGES[H].cp[1]=parseFloat(n[H][3]),a.CHALLENGES[H].cp[2]=parseFloat(n[H][4]),a.CHALLENGES[H].cp[3]=parseFloat(n[H][5]),a.CHALLENGES[H].cp[4]=
parseFloat(n[H][6]));CHALLENGE=!0;FIREBASE&&firebase.analytics().logEvent("challenge_accepted",{url:b})}catch(K){}}};this.cSave=function(b,m){var n=new Date;n.setTime(n.getTime()+1404E8);n="expires="+n.toUTCString();document.cookie=b+"="+m+";"+n+";path=/"};this.cLoad=function(b){b+="=";for(var m=document.cookie.split(";"),n=0;n<m.length;n++){for(var k=m[n];" "==k.charAt(0);)k=k.substring(1);if(0==k.indexOf(b))return k.substring(b.length,k.length)}return""};this.checksave=function(){var b=a.cLoad("OPERATOR_ID");
OPERATOR_ID=0!==b.length&&void 0!=b?b:"operator";b=a.cLoad("MODEL");var m=a.cLoad("POWER"),n=a.cLoad("DRAG"),k=a.cLoad("SENSITIVITY"),E=a.cLoad("EASE");b=0!=b.length&&void 0!==b?parseInt(b):vehicle.MODEL;m=0!==m.length&&void 0!==m?parseInt(m):vehicle.param().power;n=0!==n.length&&void 0!==n?parseInt(n):vehicle.param().drag;k=0!==k.length&&void 0!=k?parseInt(k):vehicle.param().sensitivity;E=0!==E.length&&void 0!==E?parseInt(E):vehicle.param().ease;vehicle.MODEL=b;vehicle.tune.power(m);vehicle.tune.drag(n);
vehicle.tune.sensitivity(k);vehicle.tune.ease(E)}};var BUILD="0.0.3";window.onload=init;window.onresize=resize;window.fullscreenchange=resize;window.focus=resize;var palette=[1052688,16777215,12698049,2894892,5308334,16740978,2393855,16722266,3424820,2201152,7168071,10066329,16756970],i;for(i in palette){var c=new THREE.Color(palette[i]);palette[i]=c}
